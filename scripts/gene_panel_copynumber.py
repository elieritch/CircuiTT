...
def main():
	...
	parser.add_argument("-min_baf", help="minimum allele frequency to call allele as heterozygous (default=0.40)", dest="minvaf", type=float, default=0.40, required=False)
	parser.add_argument("-max_baf", help="maximum allele frequency to call allele as heterozygous (default=0.60)", dest="maxvaf", type=float, default=0.60, required=False)
	parser.add_argument("-min_rd", help="minimum read depth to call allele as heterozygous (default=100)", dest="read_depth", type=int, default=100, required=False)
	...
	def make_b_allele_table(df, minfrequency, maxfrequency, readdepth):
		...
		#actually now removing from output
		...
		df_het = df_het.query('(Major_freq_n > @minfreq) and (Major_freq_n < @maxfreq)').query('(Minor_freq_n > @minfreq) and (Minor_freq_n < @maxfreq)')
		df_het = df_het[['chrom', 'pos', 'ref', 'Log2.depth.ratio', 'n_zygosity', 'Major_freq_n', 'Minor_freq_n', 'Minor_Allele', 'Major_Allele', 'B_allele_freq', 'gene']].reset_index(drop=True)
		return df_het
	...
	mean_gene_table['color'] = "green"
	mean_gene_table.loc[mean_gene_table['mean.gene.ndr'] >= 0.7, 'color'] = "bright red"
	mean_gene_table.loc[((mean_gene_table['mean.gene.ndr'] >= 0.4) & (mean_gene_table['mean.gene.ndr'] < 0.7)), 'color'] = "peachy pink"
	mean_gene_table.loc[((mean_gene_table['mean.gene.ndr'] > -0.4) & (mean_gene_table['mean.gene.ndr'] < 0.4)), 'color'] = "black"
	mean_gene_table.loc[((mean_gene_table['mean.gene.ndr'] <= -0.4) & (mean_gene_table['mean.gene.ndr'] > -0.7)), 'color'] = "periwinkle"
	mean_gene_table.loc[((mean_gene_table['mean.gene.ndr'] <= -0.7)), 'color'] = "strong blue"
	...
	main_graphing_table['gene'] = main_graphing_table['gene'].astype(str)
	# not sure how a . gene assignment remained in here, but it does on samples 
	# with windows CRLF line endings
	main_graphing_table = main_graphing_table.query('(gene != ".")').reset_index(drop=True)
	...
	sns.set()
	sns.set_style("whitegrid")
	fig, ax = plt.subplots(figsize=(18,10), nrows=2, sharex='all', gridspec_kw={"height_ratios":[1,0.6]})

	#automatically adjust ylim (the ceiling (floor) of the max (min) mean log2-ratio across all genes is used to set ylim,; 
	# if this is above (below) 3 (-3), then ylim is set between -3 and 3 as default. 
	# A buffer of 1 is added to nearly guarantee the violins won't get cut off by the graph's edge in either case).
	...
	#apply shading by chrom on both plots and annotate the top plot
	...
	for i in range(0,len(shade_table_2.index)):
		...
		if (i % 2 == 0): #even 
			ax[0].axvspan(min_number-0.5, max_number-0.5, color='0.75', alpha=0.4, zorder=0)
			ax[1].axvspan(min_number-0.5, max_number-0.5, color='0.75', alpha=0.4, zorder=0)
		xlocation = min_number + ((max_number-min_number)/2)
		chromosome_name = shade_table_2['chrom'][i]
		ax[0].annotate(chromosome_name, xy=(xlocation, ymax), xytext=(xlocation-0.5, ymax+0.2), size=12, ha="center",	va="center")

	#graph the violin plot
	genecolor = main_graphing_table[["gene", "color"]].drop_duplicates().reset_index(drop=True)
	color_list = genecolor['color'].tolist()
	col_list_palette = sns.xkcd_palette(color_list)
	col_pal = dict(zip(genecolor['gene'], col_list_palette.as_hex()))
	sns.violinplot(x=main_graphing_table["gene"], y=main_graphing_table["Log2.depth.ratio"], data=main_graphing_table, scale="width", width=1, cut=0, bw=.3, linewidth=0, inner=None, split=False, ax=ax[0], palette=col_pal)
	ax[0].set_ylabel(r'$\log_2$ Depth Ratio', fontsize=17)
	ax[0].set_xlabel('')
	ax[0].set_xticklabels(genecolor['gene'])
	ax[0].tick_params(axis = 'x', which="major", labelbottom=True, length=0, labelsize=15, pad=1, labelrotation=90, reset=False)
	title = str(basename+' CNV Violin Plot')
	ax[0].set_title(title, fontsize=22, pad=8)

	#graph the snp plot
	#need all genes same in both plots, even if genes dont contain snps
	df_het_graph = df_het[["gene", "B_allele_freq"]] #make table
	df_het_graph = pd.merge(genecolor, df_het_graph, how='left', on='gene') #merge with previous table to same genes.
	df_het_graph["B_allele_freq"] = (0.5 - df_het_graph["B_allele_freq"]).abs() #deviation from 0.5
	sns.stripplot(x=df_het_graph["gene"], y=df_het_graph["B_allele_freq"], data=df_het_graph, linewidth=1, color="0.5", edgecolor="0.2", ax=ax[1], jitter=False, size=7)
	ax[1].set_ylabel('B allele frequency\n(deviation from 0.5)', fontsize=17, multialignment="center", linespacing=1.4)
	ax[1].set_xlabel('')
	ax[1].set_xticklabels(genecolor['gene'])
	ax[1].tick_params(axis = 'x', which="major", labelbottom=True, length=0, labelsize=15, pad=1, labelrotation=90, reset=False)
	ax[1].set_ylim(ymin=0, ymax=0.5)

	#global setting boundaries and render
	sns.despine()
	fig.subplots_adjust(hspace=0.3, wspace=0.3, top=0.95, bottom=0.12, left=0.06, right=0.99)
	print("Rendering "+basename+"_CN_violinplot.png")
	plt.savefig(args.output+"_CN_violinplot.png", dpi=500)
