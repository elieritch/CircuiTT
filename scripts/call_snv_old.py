...
def main():
	...
	def assign_zygosity(df):
		...
		df['n_zygosity'] = np.where( ((df['TAF_n'] > 0.30) & (df['TAF_n'] < 0.95)) | ((df['CAF_n'] > 0.30) & (df['CAF_n'] < 0.95)) | ((df['GAF_n'] > 0.30) & (df['GAF_n'] < 0.95)) | ((df['AAF_n'] > 0.30) & (df['AAF_n'] < 0.95)), 'het', 'hom')
		#have to add a line if tumor base is not >0.3 and <0.7 and normal base is het then tomur zygosity is loh. 
		#for some reasons, the np.where converted some categories to strings, so put pack and collect garbage
